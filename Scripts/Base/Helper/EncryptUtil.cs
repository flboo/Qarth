...
namespace Qarth
{
    public static class EncryptUtil
    {
        ...
        /// <param name="iv"></param>
        /// <returns></returns>
        public static byte[] AESEncrypt_CBC(string text, string key, string iv)
        {
            SymmetricAlgorithm rijndaelCipher = SymmetricAlgorithm.Create("AES");

            rijndaelCipher.Mode = CipherMode.CBC;
            rijndaelCipher.Padding = PaddingMode.None;
            byte[] pwdBytes = Compress(text);

            int blockSize = rijndaelCipher.BlockSize;
            ...
            System.Array.Copy(pwdBytes, 0, plaintext, 0, pwdBytes.Length);

            rijndaelCipher.Key = System.Text.Encoding.UTF8.GetBytes(key);
            rijndaelCipher.IV = System.Text.Encoding.Default.GetBytes(iv);
        ...
        /// <summary>
        /// 解密: AES CBC NoPadding GZip减压
        /// </summary>
        /// <param name="text"></param>
        /// <param name="key"></param>
        /// <param name="iv"></param>
        /// <returns></returns>
        public static string AESDecrypt_CBC(byte[] encryptedData, string key, string iv)
        {
            SymmetricAlgorithm rijndaelCipher = SymmetricAlgorithm.Create("AES");
            ...
            using (var compressStream = new MemoryStream(bytes))
            {
                using (var zipStream = new GZipStream(compressStream, CompressionMode.Decompress))
                {
                    using (var resultStream = new MemoryStream())
                    {
                        ...
                        return Encoding.UTF8.GetString(resultStream.ToArray());//resultStream.ToArray();//
                    }
                }
            }
        }
